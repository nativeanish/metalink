...
Handlers.add("register", Handlers.utils.hasMatchingTag("Action", "register"), function(msg)
    ...
    if type(msg.Tags.status) ~= nil and msg.Tags.status == "change" then
        for i, v in ipairs(State[msg.From]) do
            if v.id == msg.Tags.id then
                -- Delete the old entry
                table.remove(State[msg.From], i)
                -- Create new entry
                table.insert(State[msg.From],
                    { id = msg.Tags.id, data = msg.Data, name = msg.Tags.name, design = msg.Tags.design })
